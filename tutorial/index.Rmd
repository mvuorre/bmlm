...
# Loads pre-fitted model to save time building the tutorial
load("~$fit.rda")
library(knitr)
...
```{r, fig.cap = "Figure 2. A coefficient plot.", fig.show='hold', fig.width=3, fig.height=3.5}
mlm_pars_plot(fit, type = "coefplot", level = .50)
mlm_pars_plot(fit, type = "coefplot", level = .99,
              p_size = 3)  # Changes size of points
```
...
```{r, fig.width = 4, fig.height = 5, fig.cap = "Participant-specific c' path values with 75% Credible Intervals."}
head(mlm_summary(fit, pars = "u_c"))
mlm_pars_plot(fit, pars = "u_cp", type = "coef", level = .75) + 
