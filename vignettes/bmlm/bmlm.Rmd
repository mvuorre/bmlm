...
The latest development version of **bmlm** is available on [GitHub](https://github.com/mvuorre/bmlm). If you don't know what a development version is, don't use one. To install R packages from GitHub, please install the `devtools` package first, as shown by the first line below. Then install **bmlm** using devtools:
...
We called the function with the argument `text = T`, showing additional transformed parameters in the upper left corner: `ab` is the average mediated effect, `c` is the total effect, and `%me` is the percent mediated effect. To disable showing the additional parameters, call the function with `text = F`.
